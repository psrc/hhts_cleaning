...
GO
		...
		WITH cte_ref AS
			...
			cte_mobile AS(
				SELECT 	t3.hhid, t3.pernum, p1.age_detailed,
					...
					JOIN HHSurvey.Person AS p1 ON t3.person_id = p1.person_id 
			...
			cte_static AS
				(SELECT t1.hhid, t1.pernum, p2.age_detailed,
					...
					JOIN HHSurvey.Person AS p2 ON t2.person_id = p2.person_id
			...
			cte_combined AS (SELECT * FROM cte_mobile UNION SELECT * FROM cte_static)
		SELECT DISTINCT * FROM cte_combined
		ORDER BY pernum;
		END
GO
